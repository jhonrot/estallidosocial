...
class Arte extends Controller
{
    public function index(Request $request)
    {
        ...
        $seconContents = Secondary_Content::where('maincontent_id', '=', 6)
                                            ->where('category_id', '=', 1)
                                            ->latest('id')
                                            ->paginate(6, ['*'], 'videos');

        $seconContents_2 = Secondary_Content::where('maincontent_id', '=', 6)
                                            ->where('category_id', '=', 2)
                                            ->latest('id')
                                            ->paginate($paginacion, ['*'], 'podcats');

        $seconds = Secondary_Content::where('maincontent_id', '=', 6)
                                    ->where('category_id', '!=', 1)
                                    ->where('category_id', '!=', 2)
                                    ->oldest('date')
        ...
        return view('arte.index', compact('maincontents', 'seconContents', 'seconContents_2', 'seconds', 'tipo', 'fecha_creacion', 'fecha_inicio', 'fecha_fin', 'palabra', 'paginacion'));
    }
}
